...
public class RoutePageServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            int countBuy= DBManager.getCountBuy(connection,  trainInfo.get("lux"));
            if(countBuy == 0){
                countBuy = DBManager.getCountBuy(connection,  trainInfo.get("coupe"));
            }

            req.setAttribute("countBuy", countBuy);
        } catch (SQLException e) {
            e.printStackTrace();
        }
