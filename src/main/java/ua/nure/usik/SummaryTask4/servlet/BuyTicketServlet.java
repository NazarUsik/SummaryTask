            ...
            if (DBManager.updateTicket(connection, ticketId, price * coefficient)) {
                if (DBManager.insertTicketsList(connection, loginedUser.getId(), ticketId, 1)) {
                    ...
                    if (DBManager.updateSeat(connection, ticket.getSeatId(), false)) {
                        ...
                        connection.setAutoCommit(false);
                        boolean ch = false;
                        System.out.println(ticket.getCountBuy());
                        Map<Pair<Integer, Float>, Pair<Integer, Integer>> allTickets = DBManager.getAllTickets(connection);
                        for (Map.Entry<Pair<Integer, Float>, Pair<Integer, Integer>> pairPairEntry : allTickets.entrySet()) {
                            if (!DBManager.updateTicket(connection, pairPairEntry.getKey().getKey(),
                                    (ticket.getCountBuy() + 1))) {
                                ch = true;
                            }
                        }
